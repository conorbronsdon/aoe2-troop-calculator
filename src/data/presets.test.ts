...
describe('Preset Army Compositions', () => {
  describe('presetCategories', () => {
    it('should have all required categories', () => {
      ...
      expectedCategories.forEach((categoryId) => {
        const category = presetCategories.find((c: PresetCategory) => c.id === categoryId);
    ...
    it('should have unique category IDs', () => {
      const ids = presetCategories.map((c: PresetCategory) => c.id);
  ...
  describe('presets', () => {
    ...
    it('should have unique preset IDs', () => {
      const ids = presets.map((p: Preset) => p.id);
      const uniqueIds = new Set(ids);
      expect(ids.length).toBe(uniqueIds.size);
    });

    it('should reference valid categories', () => {
      const validCategories = presetCategories.map((c: PresetCategory) => c.id);
      presets.forEach((preset: Preset) => {
    ...
    it('should have valid unit IDs in compositions', () => {
      const validUnitIds = units.map((u: UnitData) => u.id);
    ...
    it('should have positive quantities for all units', () => {
      presets.forEach((preset: Preset) => {
        Object.entries(preset.composition).forEach(([_unitId, quantity]) => {
    ...
    it('should have valid civilization IDs in recommendedCivs', () => {
      const validCivIds = civilizations.map((c: { id: string }) => c.id);
  ...
  describe('getPresetsByCategory', () => {
    it('should return presets for castle-age-rushes category', () => {
      ...
      castleRushes.forEach((preset: Preset) => {
    ...
    it('should return presets for imperial-compositions category', () => {
      ...
      imperialComps.forEach((preset: Preset) => {
    ...
    it('should return presets for civ-specific category', () => {
      ...
      civSpecific.forEach((preset: Preset) => {
    ...
    it('should return presets for beginner category', () => {
      ...
      beginner.forEach((preset: Preset) => {
  ...
  describe('getPresetsForCiv', () => {
    it('should return presets recommended for Franks', () => {
      ...
      const hasFranksSpecific = franksPresets.some((p: Preset) =>
        p.recommendedCivs.includes('franks')
      );
    ...
    it('should return presets recommended for Britons', () => {
      ...
      const hasBritonsSpecific = britonsPresets.some((p: Preset) =>
        p.recommendedCivs.includes('britons')
      );
    ...
    it('should return beginner presets for generic civilization', () => {
      ...
      const hasBeginnerPresets = genericPresets.some((p: Preset) => p.category === 'beginner');
    ...
    it('should return beginner presets when civId is empty', () => {
      ...
      const hasBeginnerPresets = emptyPresets.some((p: Preset) => p.category === 'beginner');
      expect(hasBeginnerPresets).toBe(true);
    });
    ...
    it('should include universal presets (empty recommendedCivs)', () => {
      ...
      const hasUniversalPresets = mayansPresets.some(
        (p: Preset) => p.recommendedCivs.length === 0
      );
  ...
  describe('preset content validation', () => {
    it('should have reasonable unit counts in compositions', () => {
      presets.forEach((preset: Preset) => {
        const totalUnits = Object.values(preset.composition).reduce(
          (sum, qty) => sum + qty,
          0
        );
  ...
  describe('preset statistics', () => {
    ...
    it('should have presets in all categories', () => {
      presetCategories.forEach((category: PresetCategory) => {
    ...
    it('should cover popular civilizations', () => {
      ...
      popularCivs.forEach((civId) => {
        const civPresets = presets.filter((p: Preset) => p.recommendedCivs.includes(civId));
