...
describe('ImportService', () => {
  ...
  describe('validateImportData', () => {
    ...
    it('should reject invalid age', () => {
      ...
      expect(result.success).toBe(false);
      expect(result.errors.some((e) => e.includes('Invalid age'))).toBe(true);
    });
    ...
    it('should warn about unknown civilization', () => {
      ...
      expect(result.success).toBe(true);
      expect(result.warnings.some((w) => w.includes('Unknown civilization'))).toBe(true);
    });
    ...
    it('should reject invalid population cap', () => {
      ...
      expect(result.success).toBe(false);
      expect(result.errors.some((e) => e.includes('Population cap'))).toBe(true);
    });
    ...
    it('should reject units without quantity', () => {
      ...
      expect(result.success).toBe(false);
      expect(result.errors.some((e) => e.includes('missing required "quantity" field'))).toBe(true);
    });
    ...
    it('should warn about unknown unit IDs', () => {
      ...
      expect(result.success).toBe(true);
      expect(result.warnings.some((w) => w.includes('Unknown unit ID'))).toBe(true);
    });
  ...
  describe('fromJSON', () => {
    it('should import valid JSON composition', () => {
      const json = JSON.stringify({
        meta: { version: APP_VERSION, age: 'imperial', civilizationId: 'britons', populationCap: 200 },
        units: [{ id: 'militia', quantity: 10 }],
      });
  ...
  describe('fromURL', () => {
    ...
    it('should fail for URL without army parameter', () => {
      ...
      const result = ImportService.fromURL(url);
      expect(result.success).toBe(false);
      expect(result.errors.some((e) => e.includes('army'))).toBe(true);
    });
    ...
    it('should fail for corrupted encoded data', () => {
      ...
      const result = ImportService.fromURL(url);
      expect(result.success).toBe(false);
      expect(result.errors.some((e) => e.includes('Failed to decode'))).toBe(true);
    });
  });
