...
import { ResourceCost, Unit, Age } from '../types';
...
vi.mock('../data/civilizations', () => ({
  getCivilizationById: vi.fn((id: string) => {
    const civs: Record<string, { name: string }> = {
...
vi.mock('../utils/calculations', () => ({
  calculateUnitCost: vi.fn((_unit: Unit): ResourceCost => ({
    food: 60,
    wood: 20,
    gold: 0,
    stone: 0,
...
describe('ExportService', () => {
  const mockComposition: Record<string, number> = { militia: 10, archer: 5 };
  ...
  describe('toCSV', () => {
    ...
    it('should skip units with zero quantity', () => {
      const composition: Record<string, number> = { militia: 0, archer: 5 };
      const csv = ExportService.toCSV(composition, mockConfig);

      const lines = csv.split('\n');
      const militiaLine = lines.find((line: string) => line.startsWith('Militia,'));
      expect(militiaLine).toBeUndefined();
    });

    it('should handle unknown civilization gracefully', () => {
      const config: ExportConfig = { ...mockConfig, selectedCiv: 'unknown' };
  ...
  describe('toJSON', () => {
    ...
    it('should include unit details', () => {
      ...
      const militia = parsed.units.find((u: { id: string }) => u.id === 'militia');
