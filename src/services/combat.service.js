 ...
 */
const getAttackType = (unit) => (unit.range > 0 ? ATTACK_TYPE.ranged : ATTACK_TYPE.melee);

/**
 * Calculate damage dealt per hit
 * AoE2 formula: damage = max(1, attack - armor)
 */
const calculateDamagePerHit = (attackerStats, defenderStats, attackType) => {
  const attack = attackerStats.attack;
  let armor;

  if (attackType === ATTACK_TYPE.ranged) {
    armor = defenderStats.pierceArmor;
  } else {
    armor = defenderStats.meleeArmor;
  }
...
/**
 * Calculate DPS (damage per second) for a unit
 */
const calculateUnitDPS = (unit, stats) => {
  const reloadTime = RELOAD_TIMES[unit.category.toLowerCase()] || 2.0;
  const damagePerHit = stats.attack;
  return damagePerHit / reloadTime;
};

/**
 * Calculate total HP for an army
 */
const calculateTotalHP = (armyUnits) => armyUnits.reduce((total, { quantity, stats }) => total + stats.hp * quantity, 0);
 ...
 */
const calculateArmyCost = (composition) => {
  const cost = { food: 0, wood: 0, gold: 0, stone: 0, total: 0 };
 ...
 */
const calculateArmyPopulation = (composition) => {
  let population = 0;
 ...
 */
const prepareArmyData = (composition, civId, researchedTechs = []) => {
  const armyUnits = [];
  ...
  Object.entries(composition).forEach(([unitId, quantity]) => {
    if (quantity > 0) {
      const unit = getUnitById(unitId);
      if (unit) {
        const unitStats = calculateUnitStats(unit, researchedTechs, civId);
        if (unitStats) {
          armyUnits.push({
            unit,
            quantity,
 ...
 *
 * @param {Object} armyA - First army { composition, civId, researchedTechs }
 * @param {Object} armyB - Second army { composition, civId, researchedTechs }
 * @returns {Object} Combat outcome analysis
 */
export const calculateCombatOutcome = (armyA, armyB) => {
  ...
  if (unitsA.length === 0 || unitsB.length === 0) {
    return {
      ...
      confidence: 'decisive',
      stats: {
        armyA: { totalHP: 0, totalDPS: 0, timeToKill: 0, cost: { total: 0 }, population: 0 },
        armyB: { totalHP: 0, totalDPS: 0, timeToKill: 0, cost: { total: 0 }, population: 0 },
      },
    };
  }
 ...
 */
export const getQuickAnalysis = (outcome) => {
