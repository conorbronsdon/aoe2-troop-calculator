...
export default function TeamBonusDisplay() {
  const { t } = useTranslation();
  const { state } = useArmy();
  const { config } = state;

  const selectedCiv = getCivilizationById(config.selectedCiv);
  const alliedCivs = config.alliedCivs.map((civId) => getCivilizationById(civId)).filter(Boolean);

  // Get all team bonuses from allies that apply to us
  const teamBonusesFromAllies = alliedCivs
  ...
  return (
    <div className="bg-gradient-to-r from-purple-50 to-indigo-50 dark:from-purple-900/20 dark:to-indigo-900/20 rounded-lg p-3 border border-purple-200 dark:border-purple-700">
      ...
      {ourTeamBonus && config.selectedCiv !== 'generic' && (
        <div>
