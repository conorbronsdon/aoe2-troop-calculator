...
interface CivilizationSelectorProps {
  ...
  previewCivId: string | null;
  onPreviewChange: (civId: string) => void;
  onApply: () => void;
}

interface ImageErrors {
  [civId: string]: boolean;
}

interface GroupedCivs {
  [region: string]: Civilization[];
}
...
export default function CivilizationSelector({
  ...
  onPreviewChange,
  onApply,
}: CivilizationSelectorProps): JSX.Element {
  ...
  const groupedCivs = filteredCivs.reduce<GroupedCivs>((acc, civ) => {
  ...
  const handleSelect = (civId: string): void => {
    onPreviewChange(civId);
    setIsOpen(false);
    setSearchTerm('');
  };

  const handleImageError = (civId: string): void => {
    setImageErrors((prev) => ({ ...prev, [civId]: true }));
  };

  const renderCivIcon = (civ: Civilization, size: string = 'w-8 h-8'): JSX.Element => {
    if (civ.id === 'generic') {
      return (
