...
interface UnitBreakdownEntry {
  ...
  quantity: number;
  baseStats: UnitStats;
  modifiedStats: UnitStats;
  bonuses: StatBreakdown;
  hasChanges: boolean;
}

interface ArmyStatsResult {
  totalUnits: number;
  totalHP: { base: number; modified: number };
  totalAttack: { base: number; modified: number };
  avgMeleeArmor: { base: number; modified: number };
  avgPierceArmor: { base: number; modified: number };
  rangedUnits: number;
  meleeUnits: number;
  avgRange: number;
...
export default function ArmyCombatStats({ embedded = false }: ArmyCombatStatsProps): React.ReactElement | null {
  const { state } = useArmy();
  const { composition, researchedTechs, config } = state;
  const [isExpanded, setIsExpanded] = useState<boolean>(true);

  // Calculate aggregate combat stats for the entire army
  const armyStats = useMemo((): ArmyStatsResult | null => {
    ...
    unitEntries.forEach(([unitId, quantity]) => {
      ...
      const { base, modified, breakdown } = unitStats as {
        base: UnitStats;
        modified: UnitStats;
      ...
      const hasChanges = (Object.keys(base) as Array<keyof UnitStats>).some(
    ...
    <>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-3">
        ...
        <div className="bg-gray-50 dark:bg-gray-700/50 rounded p-2">
          ...
          <div className="text-lg font-bold text-gray-900 dark:text-gray-100">
            {armyStats.avgMeleeArmor.modified.toFixed(1)}/{armyStats.avgPierceArmor.modified.toFixed(1)}
          </div>
      ...
      {armyStats.unitBreakdown.length > 0 && (
        <div className="border-t border-gray-200 dark:border-gray-600 pt-2">
          <div className="text-xs font-semibold text-gray-700 dark:text-gray-300 mb-2">Unit Breakdown</div>
            ...
            {armyStats.unitBreakdown.map(({ unitId, unitName, quantity, modifiedStats, hasChanges }) => (
              ...
              >
                <span className="font-medium text-gray-900 dark:text-gray-100">
                  {quantity}x {unitName}
                  {hasChanges && <span className="ml-1 text-green-600 dark:text-green-400">✨</span>}
                </span>
                <span className="text-gray-600 dark:text-gray-400">
                  HP: {Math.round(modifiedStats.hp * quantity)} | ATK: {modifiedStats.attack * quantity}
                </span>
              </div>
            ))}
          </div>
      ...
      {armyStats.hasModifications && researchedTechs.length > 0 && (
        <div className="mt-3 p-2 bg-green-50 dark:bg-green-900/20 rounded border border-green-200 dark:border-green-800">
          ...
          <div className="text-xs text-green-600 dark:text-green-400">
            Your army benefits from researched upgrades.
          </div>
        </div>
      )}
    </>
      ...
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-sm font-bold text-gray-900 dark:text-gray-100 flex items-center gap-2">
          ...
          {armyStats.hasModifications && (
            <span className="text-xs text-green-600 dark:text-green-400 font-normal">(Modified by Techs)</span>
          )}
        </h3>
        <button
