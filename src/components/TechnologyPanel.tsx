...
import { Age, ResourceCost } from '../types';
 ...
 */
export default function TechnologyPanel(): JSX.Element {
  ...
  const handleTechToggle = (techId: string): void => {
  ...
  const resetAllTechs = (): void => {
    dispatch({ type: ACTION_TYPES.RESET_TECHS });
  };

  const getCategoryIcon = (category: TechCategory | string): string => {
            ...
            <div className="flex flex-wrap gap-2">
              {Object.values(TECH_CATEGORIES)
                .filter((cat) => {
                  ...
                  const standardCats: TechCategory[] = [
                    TECH_CATEGORIES.BLACKSMITH,
                    TECH_CATEGORIES.UNIVERSITY,
                    TECH_CATEGORIES.MONASTERY,
                    TECH_CATEGORIES.STABLE,
