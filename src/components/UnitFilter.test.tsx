...
describe('UnitFilter', () => {
  const mockOnFilterChange = vi.fn<[FilterState], void>();

  // Helper to expand the filter (starts collapsed by default)
  const expandFilter = (): void => {
  ...
  describe('Search input', () => {
    it('should update search term on input', () => {
      render(<UnitFilter onFilterChange={mockOnFilterChange} />);
      expandFilter();
      const input = screen.getByPlaceholderText('Search units by name...') as HTMLInputElement;
