...
interface ImportModalProps {
  isOpen: boolean;
  onClose: () => void;
  onImport: (data: ImportData, mode: string, source: string, filename?: string) => void;
}
...
export default function ImportModal({ isOpen, onClose, onImport }: ImportModalProps): React.ReactElement | null {
  const [activeTab, setActiveTab] = useState<TabType>('file');
  const [importMode, setImportMode] = useState<string>(IMPORT_MODES.REPLACE);
  ...
  const processFileImport = async (): Promise<void> => {
    if (!selectedFile) {
      setValidationResult({
        success: false,
    ...
    if (result.success && result.data) {
      ...
      if (sanitizedData) {
        onImport(sanitizedData as ImportData, importMode, 'file', selectedFile.name);
        handleClose();
      }
    }
    setIsProcessing(false);
  ...
  const processPasteImport = (): void => {
    ...
    if (result.success && result.data) {
      const sanitizedData = ImportService.sanitizeData(result.data);
      if (sanitizedData) {
        onImport(sanitizedData as ImportData, importMode, 'paste');
        handleClose();
      }
    }
    setIsProcessing(false);
  ...
  const processUrlImport = (): void => {
    ...
    if (result.success && result.data) {
      const sanitizedData = ImportService.sanitizeData(result.data);
      if (sanitizedData) {
        onImport(sanitizedData as ImportData, importMode, 'url');
        handleClose();
      }
    }
    setIsProcessing(false);
            ...
            <div className="flex gap-4">
              <label className="flex items-center">
                <input
                  ...
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => setImportMode(e.target.value)}
