...
export default function ResourceTracker() {
  ...
  return (
    ...
    >
      ...
      {config.resourceLimitMode === 'total' && (
        <div className="mb-4">
          <div className="flex justify-between text-sm mb-1">
            <span className="font-medium" id="total-resources-label">
              Total Resources
            </span>
            <span
              className={
                totalResourcesUsed > config.totalResourceLimit ? 'text-red-600 font-bold' : ''
              }
              aria-live="polite"
            >
      ...
      {config.resourceLimitMode === 'individual' && (
        <div className="space-y-3 mb-4">
          {RESOURCES.map((resource) => (
            <div key={resource}>
              <div className="flex justify-between text-sm mb-1">
                <span className="font-medium" id={`${resource}-label`}>
                  {RESOURCE_DISPLAY_NAMES[resource]}
                </span>
                <span
                  className={
                    combinedCost[resource] > config.resourceLimits[resource]
      ...
      <div>
        <div className="flex justify-between text-sm mb-1">
          <span className="font-medium" id="population-label">
            Population
          </span>
          <span
            ...
            aria-live="polite"
          >
            {totalPopulation} / {config.populationCap}({populationPercentage.toFixed(1)}%)
          </span>
        </div>
        <div
          className="w-full bg-gray-200 rounded-full h-4 overflow-hidden"
