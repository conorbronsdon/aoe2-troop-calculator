...
import { ResourceCost, ResourceType } from '../types';

const STORAGE_KEY = 'aoe2-resource-tracker-expanded';

const RESOURCE_GRADIENT_COLORS: Record<ResourceType, string> = {
...
const getStatusIndicator = (percentage: number, isOverLimit: boolean): StatusIndicatorResult => {
  if (isOverLimit) {
...
interface ResourcePercentages {
  [key: string]: number | undefined;
  total?: number;
}

export default function ResourceTracker(): JSX.Element {
  useTranslation();
  const { state } = useArmy();
  ...
  const resourcePercentages = useMemo((): ResourcePercentages => {
    if (config.resourceLimitMode === 'total') {
      ...
      return RESOURCES.reduce<ResourcePercentages>((acc, resource) => {
          ...
          <div className="flex justify-between items-center text-xs mb-1">
            ...
            <div className="flex items-center gap-1">
              ...
              <span className="text-xs font-medium bg-gray-100 dark:bg-gray-700 px-1.5 py-0.5 rounded ml-1">
                {(resourcePercentages.total || 0).toFixed(0)}%
              </span>
            </div>
          </div>
          <div
            ...
            aria-valuenow={Math.min(100, resourcePercentages.total || 0)}
            aria-valuemin={0}
            aria-valuemax={100}
          >
            <div
              className={`h-full bg-gradient-to-r ${
                totalResourcesUsed > config.totalResourceLimit
                  ...
                  : (resourcePercentages.total || 0) >= 80
                    ? 'from-yellow-400 to-orange-500'
                    : 'from-green-400 to-emerald-500'
              } transition-all duration-300 rounded-full`}
              style={{ width: `${Math.min(100, resourcePercentages.total || 0)}%` }}
            />
          </div>
            ...
            return (
              <div key={resource}>
                ...
                <div
                  ...
                  aria-valuenow={Math.min(100, resourcePercentages[resource] || 0)}
                  aria-valuemin={0}
                  aria-valuemax={100}
                >
                  <div
                    ...
                    style={{ width: `${Math.min(100, resourcePercentages[resource] || 0)}%` }}
                  />
                </div>
              </div>
            );
