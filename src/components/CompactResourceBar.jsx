...
export default function CompactResourceBar() {
  ...
  const resourcePercentage = useMemo(() => {
    if (config.resourceLimitMode === 'total') {
      ...
      const percentages = RESOURCES.map((resource) =>
  ...
  const isResourceOverLimit = useMemo(() => {
    if (config.resourceLimitMode === 'total') {
      return totalResourcesUsed > config.totalResourceLimit;
    } else {
      return RESOURCES.some(
        (resource) => combinedCost[resource] > config.resourceLimits[resource]
      );
    }
  }, [
    config.resourceLimitMode,
